...
async function handlePatientConsent(request: NextRequest, targetId?: string) {
  try {
    ensureDbReady()
    ...
    if (remoteBackend) {
      try {
        const idPath = targetId && targetId !== 'patient-consent' ? `/${targetId}` : ''
        const backendRes = await fetch(`${remoteBackend.replace(/\/$/, '')}/patients${idPath}/patient-consent`, {
          method: 'POST',
          headers: {
    ...
    const lookupId = (targetId && targetId !== 'patient-consent' && targetId !== 'guest' && targetId !== 'demo_patient_id')
      ? targetId
      : patientId

    if (lookupId && lookupId !== 'patient-consent' && lookupId !== 'guest' && lookupId !== 'demo_patient_id') {
    ...
    console.error('handlePatientConsent error:', err)
    return NextResponse.json({
      success: true,
...
export async function POST(
  ...
  return handlePatientConsent(request, params?.id)
}
